...
public class ErimpAlter {
    ...
    public static void main(String[] args) {
        ...
        int end = sc.nextInt();
        int count = 0;
        ErimpRenges(start, end, count);

    }

    //final cheak for erimp in renge...
    static void ErimpRenges(int st, int ed, int count) {
        ...
        if (st != Reverse(st, rev)) {
            if (isPrimeN(st, st / 2) && (isPrimeN(Reverse(st, rev), Reverse(st, rev) / 2))) {
                ...
                //logic for alternative prime number...
                if (count % 2 !=0) {
